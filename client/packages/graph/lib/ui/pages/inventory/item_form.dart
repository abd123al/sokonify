...
class ItemForm<T extends PricingCubit> extends StatefulWidget {
  const ItemForm({
    Key? key,
    required this.item,
    required this.id,
...
class _ItemFormState<T extends PricingCubit> extends State<ItemForm<T>> {
  ...
  Widget build(BuildContext context) {
    return _buildForm<T>(context);
  }

  Widget _buildForm<T extends PricingCubit>(BuildContext context) {
    final priceCubit = BlocProvider.of<T>(context);

    return BlocConsumer<T, NewPrice>(
